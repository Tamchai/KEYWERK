...
var ErrActiveStripeCheckout = errors.New("order has an active Stripe Checkout session")

type OrderRepository interface {
	Create(order dto.Order, items []dto.OrderItem, cartID string) error
	...
	UpdateStatus(orderID string, fromStatus, toStatus dto.OrderStatus) error
